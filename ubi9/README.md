...
export TAG="quay.io/trevorbox/azp-agent-ubi9:1.0.3"
...
export TAG="quay.io/trevorbox/azp-agent-ubi9-minimal:1.0.3" # replace with your tag
export BASE_IMG_REPO="registry.access.redhat.com/ubi9-minimal"
export BASE_IMG_TAG="latest"

podman build -t $TAG . -f Containerfile-minimal \
  --secret=id=azp_url,env=AZP_URL \
  --secret=id=azp_token,env=AZP_TOKEN \
  --build-arg git_origin_url=$(git config --get remote.origin.url) \
  --build-arg git_revision=$(git rev-parse HEAD) \
...
```sh
export TAG="quay.io/trevorbox/azp-agent-ubi9-gator:0.0.2"
...
```sh
export TAG="quay.io/trevorbox/azp-agent-ubi9-minimal-gator:0.0.2"
podman build -f Containerfile-minimal-gator -t $TAG
podman push $TAG
```
